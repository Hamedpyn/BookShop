...
export default function Books() {
  ...
  const PaginateBooks = useCallback((buttonNum) => {
      ...
      <div className="flex items-center justify-center w-full mt-24 gap-2 px-2 flex-wrap">
        ...
        {pagesNumber.map(number => (
          <button
            ...
            key={number + 1}
            className={`border w-[65px] h-[65px] transition-all outline-none hover:text-white hover:bg-current rounded-full text-sm ${
              currentPage === number + 1 ? "bg-current text-white" : "bg-white text-current"
            }`}
          >
            {number + 1}
          </button>
